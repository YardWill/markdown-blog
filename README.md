### markdown-blog
This is a blog using markdown. It's so easy to deply. Just start with a node environment V => 7.6.0.(because of using aysnc/await)
...
And CI is to heavy too deploy this.
...
const server = http.createServer((req, res) => {
    if (req.url === '/blog' && req.method === 'POST') {
        exec('cd /data/blog && yarn run restart');
    }
    res.end('success');
});
